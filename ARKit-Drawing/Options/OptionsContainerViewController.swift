...
// Протокол определяет каркас какого либо типа
// И если какой либо тип поддерживает протокол
// То это говорит о том, что он реализовал
// в данном случае эти 4 функции
protocol OptionsViewControllerDelegate: class {
    
    // В данном случае мы обязаны реализовать эти 4 метода
    // иначе будет ошибка
...
class OptionsContainerViewController: UIViewController, UINavigationControllerDelegate {
    ...
    private func colorPicker() -> UIViewController {
        let colors: [(String, UIColor)] = [("Red", .red),
                                           ("Yellow", .yellow), ("Orange", .orange), ("Green", .green), ("Blue", .blue), ("Brown", .brown), ("White", .white),
                                           ("Black", .black)]
